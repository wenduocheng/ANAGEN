...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, conv_shortcut=False, dropout=0, ks=None, ds=None):
        super().__init__()
        self.in_channels = in_channels
        out_channels = in_channels if out_channels is None else out_channels
        self.out_channels = out_channels
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels=3, channels=[16, 16, 16, 32, 64, 64], attn_resolutions=[32], resolution=32, dropout=0, num_res_blocks=2, z_channels=128,f_channel=1000,num_class=36, **kwargs):
        super(Encoder, self).__init__()
        # channels = [c * 5 for c in channels]
        layers = [nn.Conv1d(in_channels, channels[0], 31, 1, padding=15)]
...
class Encoder_v2(nn.Module):
    ...
    16 16 16 32 64 64
    """
    def __init__(self, in_channels=3, channels=[16, 32,  64], attn_resolutions=[32], resolution=32, dropout=0.2, num_res_blocks=2, z_channels=128, f_channel=1000,num_class=36, ks=None, ds=None,downsample=False,seqlen=500):
    # def __init__(self, in_channels=3, channels=[256, 128, 128, 64, 64, 64], attn_resolutions=[32], resolution=32, dropout=0, num_res_blocks=2, z_channels=128, f_channel=1000,num_class=36, ks=None, ds=None,downsample=False):
        ...
        for i in range(len(channels) - 1): # i=5
            ...
            for j in range(num_res_blocks): # 2
                layers.append(ResnetBlock(in_channels=in_channels, out_channels=out_channels, dropout=dropout, ks=ks[count:count+2] if ks is not None else None, ds=ds[count:count+2] if ds is not None else None))
                count+=2
                in_channels = out_channels
                # if True:#resolution in attn_resolutions:
                #     layers.append(AttnBlock(in_channels))
            ...
            if downsample:
                if i < len(channels)- 2:
                    layers.append(Downsample(channels[i + 1], with_conv=True))
                    resolution //= 2
        layers.append(ResnetBlock(in_channels=channels[-1], out_channels=channels[-1], dropout=dropout, ks=ks[count:count+2] if ks is not None else None, ds=ds[count:count+2] if ds is not None else None))
        count+=2
        # layers.append(AttnBlock(channels[-1]))
        layers.append(ResnetBlock(in_channels=channels[-1], out_channels=channels[-1], dropout=dropout, ks=ks[count:count+2] if ks is not None else None, ds=ds[count:count+2] if ds is not None else None))
        count+=2
    ...
    def forward(self, x, return_embeddings=False):
        ...
        out = x.reshape(x.shape[0],-1)#transpose(1,2)
...
class Encoder_v3(nn.Module):
    ...
    # def __init__(self, in_channels=3, channels=[256, 128, 128, 64, 64, 64], attn_resolutions=[32], resolution=32, dropout=0, num_res_blocks=2, z_channels=128, f_channel=1000,num_class=36, ks=None, ds=None,downsample=False):
        ...
        self.dnaemb=nn.Embedding(4,768)
